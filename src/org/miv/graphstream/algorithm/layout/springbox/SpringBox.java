 ...
 */
package org.miv.graphstream.algorithm.layout.springbox;

import java.io.*;
import java.util.*;

import org.miv.graphstream.algorithm.layout.*;
import org.miv.util.*;
 ...
 * 
 * <p>
 * You input the graph using methods like {@link #addNode(String)}, {@link
 * #removeNode(String)}, {@link #addEdge(String,String,String,boolean)} and
 * {@link #removeEdge(String)}.
 * </p>
 * 
 * <p>
 * To be notified of the layout changes dynamically, you must register a
 * SpringBoxListener to the spring box.
 * </p>
 * 
 * <p>
 * The spring box can produce layouts both in two or three dimensions. It can
 * use a "space map", that is a division of space in cells so that when a node
 ...
 * distance. The original algorithm searches in the entire space, this is more
 ...
 * all the nodes in distant cells. This setting also decrease the layout quality
 * but speed it up a lot. When the layout becomes acceptable, it is possible to
 * desactivate this setting to refine the layout and produce very pleasant
 * results.
 * </p>
 * 
 * <p>
 * When using the space-map, the spring algorithm is very easily threadable. By
 ...
 * </p>
 * 
 * TODO: actually the space-map is fixed, adding a quad-tree/oct-tree approach
...
// Attributes
	...
	/**
	 * Strictly checks nodes are not nuplicated or edges are not duplicated.
	 */
...
// Accessors
	...
	/**
	 * Does the springbox checks duplicated nodes or edges ?
	 */
	...
	/**
	 * The edge with the identitifier id or null if no edge has this identifier.
	...
	{
		return( (double) nodeMoved / (double) nodes.size() );
	}
...
//		int n = nodesArray.size();
	 ...
	 * layout stabilizes. You can know if the layout is stable by using the
	 ...
	 * sufficiently small) the graph layout stabilized.
	 * 
	...
	public void compute()
	{
		...
		// System.err.printf( "tGlob=%f tnodes=%f tcommit=%f tbounds=%f
		// tunmap=%f%n",
		// (t2-t1)/1000000f,
		// (t3-t1)/1000000f,
		// (t4-t3)/1000000f,
	 ...
	 * is not moved. The node movement is done in a later step, see
	 * {@link #commitNode(gov4.SpringBox.CheckedNode)}.
	 * 
	 * XXX Functionnal, should be in the node class!!!!.
	 */
	protected void stepNode( Node node )
	{
		...
		// if( n > 0 )
		{
			...
			for( int i = 0; i < n; ++i )
				nodeAttraction( node, node.neighbours.get( i ).getOpposite(
						node ) );
	...
	protected void commitNode( Node node )
	{
		...
		float glueThresold = ( hi.x - lo.x ) / 10000;
	...
	/**
	 * Compute the node attraction. Functionnal, should be in the node
	 * class!!!!.
	 */
	...
	/*
	 * protected void nodeRepulsion( Node node, Node other ) { float vx, vy, vz,
	 * d, fr;
	 * 
	 * if( ! other.hiden ) { vx = node.x - other.x; vy = node.y - other.y; vz =
	 * node.z - other.z;
	 * 
	 * if( box.in3DMode() ) d = (float)Math.sqrt( ( vx * vx ) + ( vy * vy ) + (
	 * vz * vz ) ) + 0.00001f; else d = (float)Math.sqrt( ( vx * vx ) + ( vy *
	 * vy ) ) + 0.00001f;
	 *  // if( d < 5*NL ) // Only if the node is not too distant (five times the
	 * nominal distance). { vx = vx / d; vy = vy / d; vz = vz / d;
	 * 
	 * fr = K2 / ( d * d );
	 * 
	 * if( layoutConnectedOnly ) fr *= 0.5;
	 * 
	 * node.anchorx += ( fr * vx ); node.anchory += ( fr * vy ); node.anchorz += (
	 * fr * vz ); } } }
	 */
	/**
	 * Compute the node repulsion. Functionnal, should be in the node class!!!!.
	 */
	...
	/**
	 * Compute the node repulsion only in its local environment. Functionnal,
	 ...
	 * node locations. Functionnal, should be in the node class!!!!.
	 */
	@SuppressWarnings("unchecked")
...
//		int n = nodesArray.size();
	...
	/*
	 * protected void degreeDelta( int delta ) { int n = nodesArray.size(); int
	 * r = 0; int a = 0;
	 * 
	 * System.err.print( "Degree: " + degree ); degree += delta;
	 * System.err.print( " -> " + degree );
	 * 
	 * if( degree < 0 ) degree = 0;
	 *  // First for all nodes to appear, place it at the barycenter of all its //
	 * neighbor nodes that where previously visible. If none of its // neighbors
	 * was visible, choose a random position in order to avoid to // put all
	 * nodes in the same zone.
	 * 
	 * for( int i=0; i<n; ++i ) { Node node = nodesArray.get( i ); boolean
	 * shouldShow = ( node.neighbours.size() >= degree );
	 * 
	 * if( shouldShow && node.hiden == true ) { boolean pos = node.barycenter(
	 * locator );
	 * 
	 * if( ! pos ) r++; } }
	 *  // Now effectively show the nodes.
	 * 
	 * for( int i=0; i<n; ++i ) { Node node = nodesArray.get( i ); node.hiden = (
	 * node.neighbours.size() < degree );
	 * 
	 * if( ! node.hiden ) a++; }
	 * 
	 * System.err.println( " ("+r+" nodes placed randomly) ("+a+" active nodes)" ); }
	 * 
	 * protected void strideDelta( int delta ) { stride += delta;
	 * 
...
// Nested classes
	...
	public static class Node
	{
		 ...
		 * neighbors. If all neighbors are hiden, get a random position in the
