 ...
 * <a href="http://en.wikipedia.org/wiki/Bellman-Ford_algorithm">Wikipedia</a>).
 * </p>
 * 
 * <h3>Warning</h3>
 * <p>
 ...
 * </p> 
 * 
 * @complexity O(VxE) time, where V and E are the number of vertices and edges respectively.
 * 
	...
	implements Algorithm
{
	...
	/**
	 * Build a new BellmanFord algorithm giving the name
	 * of the weight attribute for edges.
	 * 
	 * @param attribute weight attribute of edges
	 */
	...
	/**
	 * Same that {@link #BellmanFord(String)} but setting
	...
	public void compute()
	{
		...
		for(int i = 0; i < graph.getNodeCount(); i++)
		{
			for(Edge e : graph.getEachEdge())
			{
				...
				if(we ==null)
					throw new NumberFormatException("org.miv.graphstream.algorithm.BellmanFord: Problem with attribute \""+weightAttribute+"\" on edge "+e);
		...
		for(Edge e : graph.getEachEdge())
		{
			...
			if( we == null )
			{
				throw new NumberFormatException(
						String.format( "%s: Problem with attribute \"%s\" on edge \"%s\"", BellmanFord.class.getName(), weightAttribute, e.getId() )
				);
			}
			...
			{
				throw new NumberFormatException(
						String.format( "%s: Problem: negative weight, cycle detected on edge \"%s\"", BellmanFord.class.getName(), e.getId() )
				);
			}
		}
	}
